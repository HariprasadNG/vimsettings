...
main(int argc, char **argv)
{
	...
	if (argc > 1  && strncmp(argv[1], "Hari", 4) == 0) {	
	   HariTest(argc, argv);
	   return;
        }
...
int HariTest(int argc, char** argv) {
    printf("My function got called %s \n",get_root());
    GTOP *gtop = gtags_open(argv[2], "/home/hari/PulseSecure", GRTAGS, GTAGS_READ, 0);
    GTP *gtp;

    for(gtp = gtags_first(gtop, NULL, /*GTOP_KEY|GTOP_NOREGEX|*/GTOP_PREFIX); gtp; gtp = gtags_next(gtop)) {
        printf("%s\t%s\t%d\n", gtp->tag, gtp->path, gtp->lineno);
    }
    gtags_close(gtop);
